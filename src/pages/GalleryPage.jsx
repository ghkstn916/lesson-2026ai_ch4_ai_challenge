...
// 카드가 viewport 안에 있을 때만 VPython iframe을 마운트한다.
// 브라우저의 WebGL 동시 컨텍스트 한도(~16)를 넘으면 오래된 iframe의 컨텍스트가
// 강제 lost되어 위로 다시 스크롤했을 때 흰 박스가 되는 문제를 막기 위해,
// viewport를 벗어나면 unmount해서 컨텍스트를 즉시 회수한다.
// CDN(jQuery·glow.js)은 첫 로드 후 브라우저 캐시에 남아 재마운트는 빠르다.
function LazyVPython({ code }) {
  ...
  return (
    <div ref={ref} style={{ marginTop: 10, minHeight: 180 }}>
      {visible ? (
        <VPythonRunner
          code={code}
          height="180px"
