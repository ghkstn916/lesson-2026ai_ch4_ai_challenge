-- 고3 인공지능 기초 8차시 — Supabase 스키마
-- PRD §2.5 기반. 운영 단위는 "개인" (teams 테이블 없음).
...
create table if not exists ai8_attempts (
  ...
  session_number int not null,              -- 1~8
  mode text not null,                       -- warmup/visual/image/structure/limit/tool/react/project
  challenge_id text not null,               -- 예: "warmup-cheer", "battle-01"
  prompt text not null,
  output_text text,                         -- 텍스트/JSON/SVG/HTML 등
  output_blob_url text,                     -- 이미지 등 바이너리 결과 URL
  variant_label text,                       -- 1차시: 역할/맥락/제약/예시 라벨
  tool_trace jsonb,                         -- 6·7차시 도구 호출 시퀀스
